 ...
 * again"; `demoOffered` records whether the shooter has been given the choice,
 * so the prompt appears once and does not nag.
 */
...
export async function loadDemoData(seed) {
  if (isWeb) {
    ...
    webWrite({
      ...
      prefs: { ...(existing.prefs || {}), seeded: true, demoOffered: true },
    });
    return webRead();
  }
  const d = await open();
  ...
  await putPref('demoOffered', true);
  return await readAll();
}
...
export async function readAll() {
  ...
  const d = await open();
  const rifleRows = await d.getAllAsync('SELECT * FROM rifles');
...
export async function putRifle(r) {
  ...
  await d.runAsync(
    `INSERT INTO rifles (id, name, cartridge, barrelLength, twist, notes,
                         ...
                         torqueJson, landsCbto, landsMeasuredAt)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
export async function putLoad(l) {
  ...
  await d.runAsync(
    `INSERT INTO loads (id, rifleId, bullet, powder, chargeGr, primer, brass,
                        ...
                        bc, dragModel, bcTruedAt, dragCurveJson)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(id) DO UPDATE SET
...
export async function putSession(s) {
  ...
  await d.runAsync(
    `INSERT INTO sessions (id, name, date, rifleId, loadId, distanceYd, distanceUnit, suppressed,
                           ...
                           projectId, projectStep, projectRowId)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(id) DO UPDATE SET
...
export async function restoreEverything(data) {
  if (isWeb) {
    webWrite({
      ...
      prefs: { ...(data.prefs || {}), seeded: true, demoOffered: true },
    });
    return webRead();
  }
