 ...
 */
export function analyseScreen(candidates, shotsPerGroup = 5, noun = 'combinations') {
  ...
  if (eliminated.length) {
    parts.push(`${eliminated.map(c => c.name).join(', ')} ${eliminated.length === 1 ? 'is' : 'are'} far enough above the ${level.toFixed(2)} MOA middle to rule out across ${k} comparisons. Drop ${eliminated.length === 1 ? 'it' : 'them'}.`);
  } else {
  ...
  parts.push(`${carryForward.length} to carry forward, deliberately unranked: ${bestSeen.name} measured smallest at ${bestSeen.groupMoa} MOA, but with one group each the order among the survivors is mostly noise. Take them all to the charge ladder, or shoot more of each before choosing.`);
