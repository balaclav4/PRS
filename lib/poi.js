...
import { rectifyToInches, project, orderCorners } from './homography.js';
...
export function targetMetrics(target, distanceYd, unit = 'MOA') {
  ...
  const H = rectifyToInches(orderCorners(scale.corners), scale.widthIn, scale.heightIn);
  ...
  metrics.poi = pointOfImpact(target, H, { x: cx, y: cy }, distanceYd, unit, metrics.centre95, n);
  // The plot needs the aim point on the same plane as the shots.
  metrics.aimIn = target?.aim ? project(H, target.aim) : null;
  return metrics;
}
 ...
 */
function pointOfImpact(target, H, centroidIn, distanceYd, unit, centre95, n) {
  const aim = target?.aim;
  if (!aim || !isFinite(aim.x) || !isFinite(aim.y)) {
    return {
      available: false,
      reason: 'No aim point recorded on this target. Mark one to measure point of impact.',
    };
  }
...
export function sessionPoi(session, unit = 'MOA') {
  ...
  if (!usable.length) {
    return {
      ...
      reason: targets.length
        ? 'No target in this session has an aim point recorded, so point of impact cannot be measured.'
  ...
  for (const m of usable) {
    const t = targets[perTarget.indexOf(m)];
    const H = rectifyToInches(orderCorners(t.scale.corners), t.scale.widthIn, t.scale.heightIn);
    const aimIn = project(H, t.aim);
    sx += (m.centroidIn.x - aimIn.x) * m.n;
    sy += (m.centroidIn.y - aimIn.y) * m.n;
    total += m.n;
  }
