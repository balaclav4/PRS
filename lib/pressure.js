...
/** Ordinary least squares fit of velocity against charge. */
export function fitLine(points) {
  const n = points.length;
  if (n < 2) return null;
  const mx = points.reduce((a, p) => a + p.charge, 0) / n;
  const my = points.reduce((a, p) => a + p.velocity, 0) / n;
  let sxy = 0, sxx = 0;
  for (const p of points) {
    sxy += (p.charge - mx) * (p.velocity - my);
    sxx += (p.charge - mx) ** 2;
  }
  if (sxx === 0) return null;
  const slope = sxy / sxx;
  const intercept = my - slope * mx;
  const residuals = points.map(p => p.velocity - (intercept + slope * p.charge));
  return {
    ...
    predict: (charge) => intercept + slope * charge,
  };
}
 ...
 */
export function analyseWorkup(points, bookMax = null) {
  ...
  const scatter = Math.max(curve.residualSd, MIN_SCATTER_FPS);
  ...
  if (bending) {
    const where = departureCharge != null
      ...
      : '';
    parts.push(`Velocity is climbing faster than the charge, not with it — the curve bends upward by ${curve.curvature.toFixed(0)} fps per grain squared (p = ${pOneSided.toFixed(3)}).${where} That bend is the signal to stop and go back to your manual.`);
  }
  if (signs.length) {
  ...
  if (!parts.length) {
    parts.push(`Velocity is tracking the charge linearly at ${line.slope.toFixed(0)} fps per grain across all ${points.length} rungs, with no upward bend (p = ${pOneSided == null ? 'n/a' : pOneSided.toFixed(2)}). That is the absence of one warning sign, not a clearance — the published maximum still governs.`);
  }
