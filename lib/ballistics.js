...
export function solve(opts = {}) {
  ...
  const table = TABLES[dragModel] || G7;
  ...
  while (x < maxRangeFt && t < 20) {
    ...
    const decel = DRAG_K * rho * cd * v * v / _bc;
...
export function dopeCard(opts = {}) {
  ...
  return {
    ...
    rows: rows.map(r => ({
      rangeYd: r.rangeYd,
      dropIn: +r.dropIn.toFixed(2),
      elevation: +conv(-r.dropIn, r.rangeYd).toFixed(2),
      windIn: +r.windIn.toFixed(2),
      wind: +conv(Math.abs(r.windIn), r.rangeYd).toFixed(2),
