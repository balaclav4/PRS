 ...
 */
export function comparePrimers(strings) {
  ...
  let verdict;
  if (significant) {
    verdict = `${best.brand} is genuinely more consistent — ${best.sd} fps against ${runnerUp.brand}'s ${runnerUp.sd} fps, a ratio the F-test puts at p = ${test.p.toFixed(3)}.`;
  } else {
    ...
    verdict = `${best.brand} measured lowest at ${best.sd} fps, but that is not a result. ${detail}${advice} Pick on availability, fit and supply.`;
  }
