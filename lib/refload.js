...
export function assessReference(opts = {}) {
  ...
  if (accuracy) {
    if (accuracy.meets) parts.push(`${groupMoa} MOA clears the ${goalMoa} MOA goal by ${accuracy.marginSigma} sigma.`);
    else if (accuracy.misses) parts.push(`${groupMoa} MOA misses the ${goalMoa} MOA goal by ${Math.abs(accuracy.marginSigma)} sigma.`);
    else parts.push(`${groupMoa} MOA against a ${goalMoa} MOA goal is inside this group's own ±${accuracy.noiseMoa} MOA noise — it neither confirms nor rules out the goal.`);
  }
