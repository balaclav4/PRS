...
export function comparisonBuckets(sessions, rifles, loads, dimension) {
  ...
  const labelFor = (s) => {
    if (dimension === 'rifles') {
      ...
      return r ? { key: r.id, label: r.name } : null;
    }
    if (dimension === 'sessions') return { key: s.id, label: s.name };
    const l = loads.find(x => x.id === s.loadId);
    return l ? { key: l.id, label: l.name } : null;
  };
  ...
  sessions.forEach(s => {
    ...
    const existing = buckets.get(id.key) || { key: id.key, label: id.label, groups: [] };
    existing.groups.push(...groups.map(g => g.inches));
...
/** Two-sample t-test between two chosen buckets. */
export function compareBuckets(a, b) {
  ...
  const sd = (arr, m) => Math.sqrt(arr.reduce((x, y) => x + (y - m) ** 2, 0) / (arr.length - 1));

  const ma = mean(a.groups), mb = mean(b.groups);
  const test = twoSampleTTest(a.groups, b.groups);
  // Order so the tighter (smaller average group) reads as the winner.
  const [lo, hi] = ma <= mb ? [a, b] : [b, a];
  const [mLo, mHi] = ma <= mb ? [ma, mb] : [mb, ma];

  return {
    error: null,
    a: { label: hi.label, mean: mHi, sd: sd(hi.groups, mHi), n: hi.n },
    b: { label: lo.label, mean: mLo, sd: sd(lo.groups, mLo), n: lo.n },
    test,
    verdict: !test
      ? 'Groups are identical — no test possible.'
      : test.significant
        ? `${lo.label} is significantly tighter — t=${Math.abs(test.t)}, df=${test.df}, p<0.05`
        : `No significant difference — t=${Math.abs(test.t)}, df=${test.df}. The ${(mHi - mLo).toFixed(2)}" gap is within normal group-to-group variation at these sample sizes.`,
  };
}
