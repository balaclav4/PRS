...
import { welchCompare, varianceCompare, dispersion, hitProbability, mean, sd } from './stats';
 ...
 */
export function compareBuckets(a, b, plateMoa = 2) {
  ...
  if (!w) {
    ...
    verdict = `${B.label} is tighter by ${Math.abs(w.diff).toFixed(2)} MOA on average (p=${fmtP(w.p)}). The true difference is likely between ${lo.toFixed(2)} and ${hi.toFixed(2)} MOA.`;
  } else {
    verdict = `No significant difference (p=${fmtP(w.p)}). The ${Math.abs(w.diff).toFixed(2)} MOA gap is within normal group-to-group variation` +
      (w.requiredN ? ` — you would need about ${w.requiredN} groups per side to settle a difference this size.` : '.');
  }

  return { error: null, a: A, b: B, welch: w, variance, verdict, plateMoa };
}
